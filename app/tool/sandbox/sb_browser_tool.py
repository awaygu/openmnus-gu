    ...
    async def _check_browser_service_health(self) -> tuple[bool, str]:
        ...
        try:
            ...
            if "PORT_NOT_LISTENING" in port_response.result or port_response.exit_code != 0:
                # 端口未监听，尝试通过 supervisorctl 重启服务进行恢复
                try:
                    logger.warning("Port 8003 not listening, attempting to restart supervisord services...")
                    restart_response = self.sandbox.process.exec("supervisorctl restart all 2>&1", timeout=60)
                    logger.info(f"Supervisorctl restart result: {restart_response.result}")

                    # 等待服务重新启动
                    time.sleep(15)

                    # 重新检查健康状态
                    recheck_response = self.sandbox.process.exec(check_cmd, timeout=10)
                    if recheck_response.exit_code == 0:
                        logger.info("Browser automation service recovered after restart!")
                        return True, "服务已通过重启恢复"

                    # 重启后仍然失败，收集诊断信息
                    diag_cmd = (
                        "echo '=== supervisorctl status ==='; supervisorctl status 2>&1; "
                        "echo '=== supervisord process ==='; pgrep -af supervisord 2>&1; "
                        "echo '=== service logs ==='; supervisorctl tail -10000 browser-automation stderr 2>&1 || supervisorctl tail all stderr 2>&1"
                    )
                    diag_response = self.sandbox.process.exec(diag_cmd, timeout=15)
                    logger.error(f"Service diagnostics after restart attempt:\n{diag_response.result}")
                except Exception as restart_e:
                    logger.error(f"Failed to restart browser service: {restart_e}")
